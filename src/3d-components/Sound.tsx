...
export function BackgroundBGM() {
    const {soundRef: rainSound, isReady: rainReady} = useSound('audio/rain.mp3', true)
    const {soundRef: bgSound, isReady: bgReady} = useSound('audio/background.mp3', true)
    const {active, type} = useOverlay(useShallow((state) => ({
      active: state.active,
      type: state.type
    })))
